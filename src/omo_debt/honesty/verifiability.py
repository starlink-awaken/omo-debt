...
def calculate_verifiability(
    ...
    verifiability_score = round(
        0.60 * evidence_score + 0.40 * trace_score,
        2
    )
...
def detect_impact_evidence(description: str, evidence_refs: list[str]) -> bool:
    ...
    evidence_keywords = [
        'error', 'bug', 'crash', 'fail', 'issue', 'user report',
        'performance', 'slow', 'latency', 'memory', 'cpu',
...
def detect_frequency_evidence(
    ...
    frequency_keywords = [
        'every', 'daily', 'weekly', 'often', 'frequently',
        'repeatedly', 'times', 'occurrences', 'hits',
        'log', 'metric', 'monitor', 'stat',
    ]
...
def detect_cost_evidence(description: str, evidence_refs: list[str]) -> bool:
    ...
    cost_keywords = [
        'hour', 'day', 'week', 'month',
        'estimate', 'effort',
        'similar', 'previous', 'last time',
        'complexity', 'difficult', 'risky',
    ]
